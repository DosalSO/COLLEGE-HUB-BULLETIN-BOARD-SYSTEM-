...
<head>
    ...
    <title>Academic Resources - College Hub</title>
    ...
    <style>
        ...
        .hero-section p { opacity: 0.88; }
        .post-card {
            border-left: 6px solid #28a745;
            border-radius: 10px;
            box-shadow: 0 2px 6px rgba(40,167,69,0.04);
            transition: box-shadow 0.18s, transform 0.18s;
        }
        .post-card:hover {
            box-shadow: 0 6px 24px rgba(40,167,69,0.11);
        ...
        .sidebar .card { border-left: 4px solid #28a745; }
        .sidebar .card-header { background-color: #28a745; color: #fff; }
        .sidebar .btn, .sidebar a { font-size: 0.99em; }
        .sidebar .fw-bold { font-weight: bold !important;}
        .sidebar .active { font-weight: bold; color: #1a6f37 !important; }
...
<body>
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <?php foreach ($categories as $category): ?>
                                <li>
                                    <a class="dropdown-item<?= $category['name'] === 'Academic Resources' ? ' active' : '' ?>" href="<?= $category['link'] ?>">
    ...
    <section class="hero-section text-center">
        <div class="container">
            <h2><i class="fas fa-book-open me-2"></i>Academic Resources</h2>
            <p class="lead mb-0">Browse and share helpful materials, guides, notes, and study resources with your fellow students.</p>
        </div>
    </section>
        ...
        <div class="row">
            <div class="col-lg-8 mb-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h5 class="mb-0 text-success"><i class="fas fa-folder-open"></i> Latest Resources</h5>
                    <a href="create_post.php?category=Academic Resources" class="btn btn-success shadow-sm">+ Create Post</a>
                </div>
                <?php if (empty($posts)): ?>
                    <div class="alert alert-info">
                        <h5>No academic resources posted yet!</h5>
                        <p>Be the first to share academic resources with your fellow students.</p>
                    </div>
                <?php else: ?>
                        ...
                        <div class="card post-card mb-4">
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <div>
                                        <span class="badge category-badge">Academic Resources</span>
                                ...
                                <p class="card-text mb-2"><?php echo nl2br(htmlspecialchars(mb_strimwidth($post['content'], 0, 180, '...'))); ?></p>
                                <div class="d-flex flex-wrap gap-3 justify-content-between align-items-center">
                                    <small class="text-muted">
                                        <i class="fas fa-user"></i> <?php echo htmlspecialchars($post['author'] ?? 'Anonymous'); ?> &nbsp;•&nbsp;
                                        <i class="fas fa-calendar-day"></i> <?php echo date('M j, Y g:i A', strtotime($post['created_at'])); ?> &nbsp;•&nbsp;
                                        <i class="fas fa-eye"></i> <?php echo isset($post['views']) ? $post['views'] : 0; ?> views
                                    </small>
                                    <a href="view_post.php?id=<?php echo $post['id']; ?>" class="btn btn-outline-success btn-sm">Read More</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <div class="col-lg-4 sidebar">
                    ...
                    <div class="card-body">
                        <?php foreach ($categories as $category): ?>
                            <a href="<?= $category['link'] ?>"
                               class="d-block text-decoration-none mb-2<?= $category['name'] === 'Academic Resources' ? ' fw-bold text-success active' : '' ?>">
                                <span class="badge me-2" style="background-color: <?= $category['color'] ?>"></span>
                                <?= htmlspecialchars($category['name']) ?>
                            </a>
                        <?php endforeach; ?>
                ...
                <div class="card mb-3 shadow quick-actions">
                    ...
                    <div class="card-body">
                        ...
                        <a href="my_posts.php" class="btn btn-outline-success w-100">📝 My Posts</a>
                    </div>
                </div>
                <div class="card mt-3 shadow-sm">
                    <div class="card-header">
                        <i class="fas fa-info-circle me-1"></i>About Academic Resources
                    </div>
                    <div class="card-body">
                        <small>
                            Share notes, e-books, guides, past exams, and helpful websites. Help others succeed by contributing valuable learning materials!
                        </small>
                    </div>
                </div>
            </div>
